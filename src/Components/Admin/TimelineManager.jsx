...
import { getTimeline, saveTimeline } from '../../utils/dataManager';
import {
  getDefaultTimeline,
  saveDefaultTimeline,
...
const TimelineManager = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setEntries((Array.isArray(data) ? data : []).map((e, i) => ({ ...e, _localId: e._localId ?? i })));
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      showStatus(result.success ? `${mode.toUpperCase()}_TIMELINE_SAVED` : result.message, !result.success);
                ...
                {editingId === entry._localId ? (
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <Field label="Year / Range" value={editForm.year || ''} onChange={e => setEditForm(p => ({ ...p, year: e.target.value }))} />
                      <Field label="Title / Role" value={editForm.title || ''} onChange={e => setEditForm(p => ({ ...p, title: e.target.value }))} />
                      <Field label="Place / Organisation" value={editForm.place || ''} onChange={e => setEditForm(p => ({ ...p, place: e.target.value }))} />
                      <Field label="Icon (Lucide name)" value={editForm.icon || ''} onChange={e => setEditForm(p => ({ ...p, icon: e.target.value }))} />
                    </div>
                    <Field label="Description" value={editForm.desc || ''} onChange={e => setEditForm(p => ({ ...p, desc: e.target.value }))} multiline />
