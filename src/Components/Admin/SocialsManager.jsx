...
import { getSocials, saveSocials } from '../../utils/dataManager';
import {
  getDefaultSocials,
  saveDefaultSocials,
...
const SocialsManager = () => {
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      showStatus(result.success ? `${mode.toUpperCase()}_SOCIALS_SAVED` : result.message, !result.success);
  ...
  const handleStartEdit = (entry) => { setEditingId(entry._localId); setEditForm({ ...entry }); };
                  ...
                  <div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <select value={editForm.icon} onChange={e => setEditForm(p => ({ ...p, icon: e.target.value }))}
                      ...
                      <Field label="Platform Label" value={editForm.label || ''} onChange={e => setEditForm(p => ({ ...p, label: e.target.value }))} />
                      <Field label="URL" value={editForm.url || ''} onChange={e => setEditForm(p => ({ ...p, url: e.target.value }))} />
                      <Field label="Username / Handle" value={editForm.username || ''} onChange={e => setEditForm(p => ({ ...p, username: e.target.value }))} />
                    </div>
