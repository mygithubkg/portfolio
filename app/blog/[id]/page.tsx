...
import { getBlogById, incrementBlogViews } from '@/lib/utils/blogData';
...
/* ═══════════════════════════════════════════════════════════════════
   DESIGN TOKENS
   Color / type / radius / spacing are centralized here so every
   section derives from the same scale rather than inventing its own.
   ═══════════════════════════════════════════════════════════════════ */
...
/* ──────────────────────────── Toast ──────────────────────────── */
/* Kept for share/bookmark feedback, but deliberately quiet — the
   ledger is the page's one loud moment, everything else stays low. */
...
/* ──────────────────────────── Heading with Ledger Index ────────────────────────────
   Replaces the old hover-anchor pattern. Each heading carries the same
   two-digit index used in the TOC — one numbering system doing
   navigation, progress, and permalinks instead of three separate ones. */
...
function BlogDetailPage() {
  ...
  /* Scroll listener — dock appears once past the hero */
  useEffect(() => {
  ...
  if (loading) {
    return (
      <section className="min-h-screen bg-[var(--bg)] flex items-center justify-center">
        <style>{TOKENS_STYLE}</style>
        <div className="flex flex-col items-center gap-4">
          <div className="w-8 h-8 border-2 border-[var(--rule-strong)] border-t-[var(--accent)] rounded-full animate-spin" />
          <span className="text-[var(--ink-faint)] font-chrome text-[11px] tracking-[0.2em] uppercase">
            Loading
          </span>
        </div>
      </section>
    ...
    return (
      <section className="min-h-screen bg-[var(--bg)] flex items-center justify-center px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-md">
          <div className="w-16 h-16 mx-auto mb-6 rounded-[12px] bg-[var(--bg-raised)] border border-[var(--rule)] flex items-center justify-center">
            <BookOpen className="text-[var(--ink-faint)]" size={26} />
          </div>
          <h1 className="font-editorial text-3xl font-semibold text-[var(--ink)] mb-3">Post not found</h1>
          <p className="text-[var(--ink-dim)] mb-8 leading-relaxed">
            The article you&apos;re looking for doesn&apos;t exist or may have been moved.
          </p>
          <button
            onClick={() => router.push('/blog')}
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-[var(--ink)] text-[var(--bg)] rounded-[8px] text-sm font-chrome font-medium hover:opacity-90 transition-opacity"
          >
            <ArrowLeft size={14} />
            Back to blog
          </button>
      ...
      <article className="min-h-screen bg-[var(--bg)] text-[var(--ink)]">
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              author: { '@type': 'Person', name: blog.author || 'Karrtik Gupta', url: 'https://www.karrtikgupta.me' },
        ...
        <header className="relative w-full overflow-hidden border-b border-[var(--rule)]">
          ...
          <div className="relative z-10 max-w-[1120px] mx-auto px-6 md:px-12 pt-24 pb-12">
            <button
              onClick={() => router.push('/blog')}
              className="inline-flex items-center gap-2 text-[var(--ink-faint)] hover:text-[var(--ink)] text-[13px] font-chrome mb-8 transition-colors group"
            >
            ...
            <motion.h1
              ...
              className="font-editorial text-4xl sm:text-5xl md:text-[3.25rem] font-semibold tracking-tight leading-[1.1] mb-6 max-w-3xl"
            >
              {blog.title}
            </motion.h1>

            <p className="font-editorial italic text-lg md:text-xl text-[var(--ink-dim)] leading-relaxed max-w-2xl mb-8">
              {blog.excerpt}
            </p>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-3 font-chrome text-[13px] text-[var(--ink-faint)]">
              <span className="flex items-center gap-2 text-[var(--ink-dim)]">
                <span className="w-7 h-7 rounded-[6px] bg-[var(--bg-raised)] border border-[var(--rule)] flex items-center justify-center text-[10px] font-medium text-[var(--ink)]">
                  {authorInitials}
                </span>
        ...
        <div className="max-w-[1120px] mx-auto px-6 md:px-12">
          <div className="xl:flex xl:justify-between xl:gap-16">
                  ...
                  components={{
                    code({ node, inline, className, children, ...props }: any) {
                      ...
                      return !inline && match ? (
                        ...
                        <code
                          className="font-chrome bg-[var(--bg-raised)] text-[var(--accent)] px-1.5 py-0.5 rounded-[4px] text-[0.82em] border border-[var(--rule)]"
                          {...props}
                        >
                          {children}
                        </code>
                    ...
                    h1: ({ children }: any) => {
                      ...
                      return <HeadingWithAnchor level={1} id={hid} index={indexById[hid] ?? null}>{children}</HeadingWithAnchor>;
                    },
                    h2: ({ children }: any) => {
                      const hid = slugify(extractText(children));
                      return <HeadingWithAnchor level={2} id={hid} index={indexById[hid] ?? null}>{children}</HeadingWithAnchor>;
                    },
                    h3: ({ children }: any) => {
                      const hid = slugify(extractText(children));
                      return <HeadingWithAnchor level={3} id={hid} index={indexById[hid] ?? null}>{children}</HeadingWithAnchor>;
                    },
                    p: ({ children }: any) => (
                      <p className="text-[var(--ink)] leading-[1.8] text-[18px] mb-6">{children}</p>
                    ),
                    ...
                    a: ({ children, href }: any) => (
                      <a
                        ...
                        className="text-[var(--accent)] underline underline-offset-4 decoration-[var(--accent-rule)] hover:decoration-[var(--accent)] transition-all"
                      >
                        {children}
                      </a>
                    ),
                    ...
                    img: ({ src, alt }: any) => (
                      <figure className="my-8 -mx-6 md:mx-0">
                        <img
                          src={src}
                          alt={alt || ''}
                          className="w-full rounded-[12px] border border-[var(--rule)]"
                          onError={(e: any) => { e.target.style.display = 'none'; }}
                        />
                        {alt && (
                          <figcaption className="text-center text-xs text-[var(--ink-faint)] font-chrome mt-3">
                            {alt}
                          </figcaption>
                        )}
                      </figure>
                    ...
                    table: ({ children }: any) => (
                      <div className="overflow-x-auto my-8 rounded-[8px] border border-[var(--rule)]">
                        <table className="w-full text-sm">{children}</table>
                      </div>
                    ),
                    th: ({ children }: any) => (
                      <th className="px-4 py-3 text-left text-[11px] font-chrome text-[var(--ink-faint)] uppercase tracking-wider bg-[var(--bg-raised)] border-b border-[var(--rule)]">
                        {children}
                      </th>
                    ),
                    td: ({ children }: any) => (
                      <td className="px-4 py-3 text-[var(--ink)] border-b border-[var(--rule)] text-[15px]">{children}</td>
                    ),
              ...
              <div className="mt-24 pt-8 border-t border-[var(--rule)]">
                {blog.hashtags && blog.hashtags.length > 0 && (
                  <div className="flex flex-wrap items-center gap-2 mb-8">
                    ...
                    {blog.hashtags.map((tag: string, i: number) => (
                      <span
                        key={i}
                        className="px-2.5 py-1 rounded-[4px] bg-[var(--bg-raised)] text-[var(--ink-faint)] text-[12px] font-chrome border border-[var(--rule)]"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 py-6 border-y border-[var(--rule)]">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-[8px] text-[13px] font-chrome transition-all border ${bookmarked
                      ? 'bg-[var(--accent-dim)] text-[var(--accent)] border-[var(--accent-rule)]'
                      : 'bg-[var(--bg-raised)] text-[var(--ink-faint)] border-[var(--rule)] hover:text-[var(--ink)]'
                      }`}
                  >
                ...
                <div className="py-8">
                  <div className="flex items-start gap-4 p-6 rounded-[16px] bg-[var(--bg-raised)] border border-[var(--rule)]">
                    <div className="w-11 h-11 rounded-[8px] bg-[var(--bg-raised-2)] border border-[var(--rule)] flex items-center justify-center text-[var(--ink)] font-chrome font-medium text-[13px] flex-shrink-0">
                      {authorInitials}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[var(--ink)] font-semibold font-editorial text-lg mb-1">
                        {blog.author || 'Karrtik Gupta'}
                      </p>
                      <p className="text-[var(--ink-dim)] text-sm leading-relaxed mb-3">
                        {blog.authorBio ||
                          'Developer, writer, and creator. Building things for the web and sharing what I learn along the way.'}
                      </p>
                      <div className="flex items-center gap-4 font-chrome text-[12px] text-[var(--ink-faint)]">
                        <a href="https://www.instagram.com/karrtik_gupta/" target="_blank" rel="noopener noreferrer" className="hover:text-[var(--accent)] transition-colors">
                          Instragram →
                        </a>
                        <a href="https://github.com/mygithubkg" target="_blank" rel="noopener noreferrer" className="hover:text-[var(--accent)] transition-colors">
                          github →
                        </a>
                      </div>
                    </div>
                ...
                <div className="text-center pt-4 pb-8">
                  <button
                    onClick={() => router.push('/blog')}
                    className="inline-flex items-center gap-2 px-6 py-3 rounded-[8px] bg-[var(--ink)] text-[var(--bg)] text-sm font-chrome font-medium hover:opacity-90 active:scale-[0.98] transition-all"
                  >
                    <ArrowLeft size={14} />
                    All articles
                  </button>
            ...
            {/* ── Desktop Ledger (TOC + scrollspy + signature element) ── */}
            {tocItems.length > 0 && (
              <aside className="hidden xl:block w-[200px] flex-shrink-0">
                <nav className="sticky top-24">
                  <p className="font-chrome text-[10px] text-[var(--ink-faint)] uppercase tracking-[0.15em] mb-4">
                    Contents
                  </p>
                    ...
                    {tocItems.map((item, i) => {
                      ...
                      return (
                        <li key={item.id} className="relative">
                          <span
                            className={`absolute -left-4 top-0 bottom-0 w-[1px] transition-colors duration-300 ${isActive ? 'bg-[var(--accent)]' : 'bg-transparent'
                              }`}
                          />
                          <a
                            href={`#${item.id}`}
                            className={`flex items-baseline gap-2.5 py-1.5 font-chrome text-[13px] leading-snug transition-colors ${item.level === 3 ? 'pl-4' : ''
                              } ${isActive ? 'text-[var(--accent)]' : 'text-[var(--ink-faint)] hover:text-[var(--ink-dim)]'}`}
                          >
        ...
        {/* ═══════════ UTILITY DOCK — one adaptive floating control instead of separate rails/drawers ═══════════ */}
        <AnimatePresence>
          {showDock && (
            <motion.div
              initial={{ opacity: 0, y: 8 }}
              ...
              exit={{ opacity: 0, y: 8 }}
              transition={{ duration: 0.15 }}
              className="fixed bottom-6 right-6 z-40 flex items-center gap-1 p-1 rounded-[10px] bg-[var(--bg-raised)]/95 backdrop-blur border border-[var(--rule)] shadow-lg"
            >
              {tocItems.length > 0 && (
                <button
          ...
          {showToc && (
            <>
              ...
              <motion.div
                ...
                className="xl:hidden fixed bottom-20 right-6 left-6 sm:left-auto sm:w-80 z-50 max-h-[60vh] bg-[var(--bg-raised)] border border-[var(--rule)] rounded-[12px] shadow-2xl overflow-hidden"
              >
                <div className="p-5">
                  <p className="font-chrome text-[10px] text-[var(--ink-faint)] uppercase tracking-[0.15em] mb-4">
                    Contents
                      ...
                      return (
                        <li key={item.id}>
                          <a
                            href={`#${item.id}`}
                            onClick={() => setShowToc(false)}
                            className={`flex items-baseline gap-2.5 px-2 py-2 rounded-[6px] text-sm transition-all ${item.level === 3 ? 'pl-6' : ''
                              } ${isActive ? 'text-[var(--accent)] bg-[var(--accent-dim)]' : 'text-[var(--ink-dim)] hover:text-[var(--ink)] hover:bg-[var(--bg-raised-2)]'}`}
                          >
